...
WIDGETS = [
    ...
    # Подписи честные: 11.08.2026 проверено переключением всех пятнадцати реле —
    # ни одно на эти токи не влияет, значит трансформаторы стоят не на
    # коммутируемых линиях. Опознан пока только насос, по совпадению с частотой.
    {"id": "wPowerCurrents", "name": "Токовые входы MAP", "description": "", "compact": False,
     "cells": cells([
         ("wb-map12e_104/Ch %d Irms L%d" % (ch, ph),
          "Ch %d · L%d%s" % (ch, ph, " — насос" if (ch, ph) == (4, 1) else ""))
         for ch in range(1, 5) for ph in range(1, 4)
     ])},
]
