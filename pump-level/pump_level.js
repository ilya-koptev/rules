// pump_level.js — управление насосом по уровню воды
// Устройство уровня : wb-mai2-mini_171 (MAI2-mini, ток. вход)
...
//
// Уровень считается по КАЛИБРОВОЧНОЙ ТАБЛИЦЕ (ток датчика, мА -> собственная
// шкала датчика, мм), кусочно-линейной интерполяцией. Собственная шкала 0…26 мм.
// Датчик установлен со смещением по высоте, поэтому:
//     level_mm = interp(ток) + OFFSET
// OFFSET задаётся вживую контролом «Смещение датчика».
// ВНИМАНИЕ: датчик насыщается — при токе > ~17 мА (собств. показания > ~14 мм)
// разрешение по уровню резко падает. Держите рабочую точку ниже.
